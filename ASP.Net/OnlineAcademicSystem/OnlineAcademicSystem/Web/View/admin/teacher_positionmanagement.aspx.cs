...
{
    public partial class teacher_positionmanagement : System.Web.UI.Page
    {
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            if(issusses==true)
            {
                Response.Write("<script>alert('有约束关系，删除失败！');</script>");
            }
            else
            {
                ub.Delete(pk);
                GridView1.DataBind();
                Response.Write("<script type=\"text/javascript\">alert('删除成功！');window.location='teacher_positionmanagement.aspx';</script>");
            }
        }
    }
}